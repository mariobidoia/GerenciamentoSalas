document.addEventListener("DOMContentLoaded", () => {
    ...
    // NOVO: Constante para mapear os dias da semana
    ...
    function renderNotifications(pendingRequests) {
        ...
        const requestsHtml = pendingRequests.map(req => {
            ...
            let recurringDaysHtml = ""; // NOVO: Variável para os dias da semana
            ...
            if (req.isRecurring && req.startDate && req.endDate) {
                ...
                // Verifica o tipo de recorrência para exibir os dias
                if (req.type === 'weekly' && typeof req.daysOfWeek === 'string' && req.daysOfWeek.length > 0) {
                    // Mapeia os números (ex: "1,3,5") para os nomes (ex: "Seg, Qua, Sex")
                    const dayNames = req.daysOfWeek.split(',')
                                         .map(Number)
                                         .map(d => weekdays[d] || '?') // Usa o array 'weekdays'
                                         .join(', ');
